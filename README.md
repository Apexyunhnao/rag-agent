...
[![CI](https://github.com/Apexyunhnao/rag-agent/actions/workflows/ci.yml/badge.svg)](https://github.com/Apexyunhnao/rag-agent/actions/workflows/ci.yml)

> 本服务是**智能客服 Agent 平台**的一部分。统一入口（编排 + 认证）见
> [orchestrator-agent](https://github.com/Apexyunhnao/orchestrator-agent)，
> 工单处理见 [customer-service-agent](https://github.com/Apexyunhnao/customer-service-agent)。
...
> 它与 orchestrator-agent / customer-service-agent 必须是**同一个值**（共享验签）——
> 最省事是在入口仓库运行 `start-demo.bat`，它会生成一次随机密钥并写入三仓 `.env`。

> 接口需要认证（HttpOnly Cookie `svc_token`）：本服务与入口服务**共用同一个密钥，
> 但独立验签**，不信任上游传来的角色字符串。演示请在 `http://127.0.0.1:8010/` 登录一次。
> 调用方传入的 `X-Trace-Id` 会被记入 `queries` 表，用于跨服务串联一次请求。
...
- 单轮问答，不支持多轮对话
- **来源正确率是薄弱项**：跨文档对比类问题（如"退货和换货有什么区别"）只标注单一来源，
  接口的 `sources` 已是数组但生成端目前只给一个
- 无独立限流（认证已补齐：与入口服务共用密钥、**本服务独立验签**）
- 知识库只有 4 份文档 8 个 chunk，覆盖度有限
